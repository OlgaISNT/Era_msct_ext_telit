...
static INT8 find_slot_by_handle(M2MB_OS_TASK_HANDLE h)
{
  ...
  for (i=0; i< AZX_TASKS_MAX_TASKS; i++)
  {
    ...
    {
    LOG_TRACE("comparing task handle %p with current slot %d handle %p", h, i, m2mb_tasks.task_slots[i].Task_H );
...
INT32 azx_tasks_sendMessageToTask( INT8 TaskProcID, INT32 type, INT32 param1, INT32 param2 )
{
  ...
  if (! m2mb_tasks.isInit)
  {
    LOG_ERROR("m2m task first init not performed yet");
  ...
  {
	  LOG_ERROR("task id %d not valid!", TaskProcID);
    return AZX_TASKS_INVALID_ID_ERR;
  }

  if(m2mb_tasks.task_slots[slot].SlotInUse == 0)
  {
	  LOG_ERROR("task id %d not existing", TaskProcID);
  ...
  LOG_TRACE( "message ==> type=%d; par1=%d; par2=%d", tmpMsg.type, tmpMsg.param1, tmpMsg.param2 );
  osRes = m2mb_os_q_tx( Queue_H, (void*)&tmpMsg, M2MB_OS_NO_WAIT, 0 );

  if( osRes != M2MB_OS_SUCCESS )
  {
	  LOG_ERROR( "Send message to Task %d failed; error %d", TaskProcID, osRes );
...
void Task_EntryFn( void *arg )
{
  ...
  if (slot == -1)
  {
	  LOG_ERROR("Cannot find slot for handle %d", taskHandle);
    return;
  }

  LOG_TRACE("slot: %d", slot);


  m2mb_os_taskGetItem( taskHandle, M2MB_OS_TASK_SEL_CMD_NAME, &task_name, NULL );

  LOG_TRACE( "==>Task %s started!", (char*) task_name);
  ...
  while( 1 )
  {
     ...
     LOG_TRACE( "%s received a message: \r\n"
             "- type   = %d\r\n"
             "- param1 = %d\r\n"
             "- param2 = %d\r\n",
  ...
  LOG_TRACE("exiting entry function. ");
}
...
INT32 azx_tasks_createTask( char *task_name, INT32 stack_size, INT32 priority, INT32 msg_q_size, USER_TASK_CB cb)
{
  ...
  {
	  LOG_ERROR("priority out of bounds");
  ...
  {
	  LOG_ERROR("message queue size out of bounds");
  ...
  LOG_TRACE("task_name_buf: %s", m2mb_tasks.task_slots[slot].Task_NameBuf);

  m2mb_tasks.task_slots[slot].Task_UserCB = cb;

  LOG_TRACE( "Create task messages queue" );
  if ( m2mb_os_q_setAttrItem( &Task_Queue_Attr_H, 1,M2MB_OS_Q_SEL_CMD_CREATE_ATTR,NULL) != M2MB_OS_SUCCESS )
  {
	  LOG_ERROR( "SET QUEUE ATTRIBUTES - FAIL  --> SKIP QUEUE CREATION !!!" );
  ...
  else
  {
    ...
    {
    	LOG_ERROR( "SET QUEUE ATTRIBUTES - FAIL" );
      m2mb_tasks.task_slots[slot].SlotInUse = 0;
      return AZX_TASKS_Q_ATTRIB_SET_ERR;
    }
    else
    {
    	LOG_TRACE( "Set queue attributes done" );

      os_res = m2mb_os_q_init( &(m2mb_tasks.task_slots[slot].Task_Queue_H), &Task_Queue_Attr_H  );
      if ( M2MB_OS_SUCCESS != os_res )
      {
    	  LOG_ERROR( "QUEUE CREATE - FAIL" );
      ...
      else
      {
        ...
        if ( M2MB_OS_SUCCESS != os_res )
        {
        	LOG_ERROR( "SET TASK ATTRIBUTES - FAIL" );
          m2mb_tasks.task_slots[slot].SlotInUse = 0;
          return AZX_TASKS_ATTRIB_SET_ERR;
        }
        else
        {
        	LOG_TRACE( "SET TASK ATTRIBUTES - PASS" );
        }

        LOG_TRACE( "Creating the task" );
        ...
        else
        {
          ...
          {
        	  LOG_ERROR( "INVALID TASK HANDLE - FAIL" );
            m2mb_tasks.task_slots[slot].SlotInUse = 0;
            return -1;
          }
          LOG_TRACE("Task created successfully with handle %p", m2mb_tasks.task_slots[slot].Task_H);
...
INT32 azx_tasks_destroyTask(INT8 TaskProcID)
{
  ...
  LOG_TRACE("Trying to terminate task...");
  res = m2mb_os_taskTerminate(m2mb_tasks.task_slots[slot].Task_H);
  if (res != M2MB_OS_SUCCESS)
  {
	  LOG_ERROR("Cannot terminate task %d", TaskProcID);
    return res;
  }

  m2mb_os_taskGetItem( m2mb_tasks.task_slots[slot].Task_H, M2MB_OS_TASK_SEL_CMD_STATE, &out, NULL );
  LOG_TRACE ( "Task state: %d", out );

  LOG_TRACE("Trying to delete task with handle %d...", m2mb_tasks.task_slots[slot].Task_H);
  res = m2mb_os_taskDelete(m2mb_tasks.task_slots[slot].Task_H);
  if (res != M2MB_OS_SUCCESS)
  {
	  LOG_ERROR("Cannot delete task %d", TaskProcID);
    return res;
  }

  LOG_TRACE("Trying to clear task queue...");
  res = m2mb_os_q_clear(m2mb_tasks.task_slots[slot].Task_Queue_H);
  if (res != M2MB_OS_SUCCESS)
  {
	  LOG_ERROR("Cannot clear task %d queue ", TaskProcID);
    return res;
  }

  LOG_TRACE("Trying to deinit task queue...");
  res = m2mb_os_q_deinit(m2mb_tasks.task_slots[slot].Task_Queue_H);
  if (res != M2MB_OS_SUCCESS)
  {
	  LOG_ERROR("Cannot deinit task %d queue ", TaskProcID);
    return res;
  }
...
INT32 azx_tasks_getEnqueuedCount( INT8 task_id )
{
  ...
  {
	  LOG_ERROR("Cannot read enqueued message count: %d", result);
    return -1;
  }
