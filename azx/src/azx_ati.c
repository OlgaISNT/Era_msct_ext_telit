...
#define WAIT_BETWEEN_READS_MS 20 // было 20 // снижение интервала позволяет избежать проблемы неопределения URC в ситуации, когда следуют несколько URCs подряд: #ECALLEV:12 и сразу же DISCONNECTED
...
void init_azx_ati(void) {
	memset(silentCmds, 0, sizeof(silentCmds));
}
      ...
      if(!silentMode)
      {
        AZX_LOG_TRACE("Cutting out URC\r\n");
      	  ...
      	  AZX_LOG_DEBUG("Reporting URC: %s\r\n", (const CHAR*)p);
      	  	  	  	  //print_hex("Reporting URC", (char *)p, strlen((const CHAR*)p));
      hnd->cb((const CHAR*)p);
      if(!silentMode)
      {
...
static BOOLEAN is_response_ok(const struct AzxBinaryData* response)
{
  ...
  size = response->size;
 // DK: иногда response->data может содержать не только <\r\nOK>, но и часть URC, отправленного сразу после <\r\nOK> и тогда код ниже даёт ошибку:
  //print_hex("response", (char *) response->data, size);
/*
  ...
  return FALSE;
*/
  // вариант, устойчивый к описанной проблеме:
  if (size >= 4 && strstr((char *)response->data, "\r\nOK") != NULL) return TRUE;
  else return FALSE;
