...
#define MAX_DUR 1800000 // максимальная задаваемая продолжительность таймера на низком уровне: 1800000 mS = 1800 sec = 30 min

/** DKOZENKOV:
 * В комментариях к функции запуска таймера Telit просто указал, что его можно запустить на UINT32 кол-во mS.
 * Т.е., формально, их библиотека должна поддерживать таймеры длительностью 4294967295 mS (0xFFFFFFFF),
 * а реально получается в 1000 раз меньше, т.к. аппаратный таймер отсчитывает время не в mS, а в uS!
 * Поэтому в эту библиотеку были внесены исправления, позволившие задавать максимальную длительность таймера в 4294967295 mS (0xFFFFFFFF)
 * Для этого в случае, если задаваемая продолжительность таймера превышает MAX_DUR, то таймер будет перезапускаться с периодичностью в MAX_DUR
 * до тех пор, пока не истечёт общее заданное время, после чего будет вызван callback
 */
...
typedef struct {
  ...
  UINT32 cycle_duration_ms; // текущая продолжительность одного цикла таймера, которая должна быть не более MAX_DUR
  UINT32 total_duration_ms; // общая задаваемая продолжительность таймера за минусом продолжительности cycle_duration_ms, которая может иметь максимум 4294967295 mS = 4294967 sec = 71582 min
...
static Timer allTimers[MAX_TIMERS];
...
static void set_duration(Timer* tim, UINT32 duration_ms) {
	UINT32 total_dur = duration_ms > 0 ? duration_ms : 1000;
	UINT32 n = total_dur / MAX_DUR;

	tim->total_duration_ms = n * MAX_DUR;
	tim->cycle_duration_ms = total_dur - tim->total_duration_ms;

	if (tim->cycle_duration_ms == 0) {
		if (n == 0) {
			tim->cycle_duration_ms = 1000;
			tim->total_duration_ms = 0;
		} else {
			tim->cycle_duration_ms = MAX_DUR;
			tim->total_duration_ms -= MAX_DUR;
		}
	}
	//LOG_INFO("total_dur: %i, total_duration_ms: %i, cycle_duration_ms: %i", total_dur, tim->total_duration_ms, tim->cycle_duration_ms);
}
...
static Timer* do_init(Timer* tim, UINT32 duration_ms) {
  ...
  //set_duration(tim, duration_ms);
  tim->total_duration_ms = duration_ms;
  res = m2mb_hwTmr_setAttrItem( &attr,
      CMDS_ARGS(
        M2MB_HWTMR_SEL_CMD_CB_FUNC, &timer_cb,
        M2MB_HWTMR_SEL_CMD_ARG_CB, tim,
        M2MB_HWTMR_SEL_CMD_TIME_DURATION, M2MB_HWTMR_TIME_MS(tim->total_duration_ms),
...
static INT32 timer_task(INT32 unused_type, INT32 timer_id, INT32 unused_param)
{
  ...
  if (timer) {
	  if (timer->total_duration_ms == 0) {
		  if (timer->cb) timer->cb(timer->ctx, timer_id);
	  } else {
		  //LOG_INFO("RESTART_TIMER: %i for duration %i mS", timer_id, timer->total_duration_ms);
		  do_start(timer, timer->total_duration_ms, TRUE);
	  }
  }
  return 0;
}
...
static void do_start(Timer* tim, UINT32 duration_ms, BOOLEAN restart) {
  ...
  if (duration_ms > 0) {
    set_duration(tim, duration_ms);
    UINT32 timeDuration = tim->cycle_duration_ms * 1000;
